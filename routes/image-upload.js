  ...
  newImage.save((err, Image) => {
    if (err) {
      ...
      User.updateOne({username: req.body.username}, {$inc: {numPosts: 1}}, function(err) {
        if(err) {
          return res.status(400).send({errors: [{title: 'Failed to increment numPost', detail: err.message}] });
        }
      });
...
router.post("/delete-image", (req, res) => {
  ...
  Image.deleteOne({imageUrl: req.body.imageUrl}, function(err) {
    if (err) {
      return res.status(400).send({errors: [{title: 'Failed to Delete', detail: err.message}] });
    } else {
      // If successful the user's numPosts is decremented by 1
      User.updateOne({username: req.body.username}, {$inc: {numPosts: -1}}, function(err) {
        if(err) {
          return res.status(400).send({errors: [{title: 'Failed to decrement numPost', detail: err.message}] });
        }
      });
      return res.status(200).send({
...
router.post("/update-image", (req,res) => {
  ...
  Image.updateOne({imageUrl: req.body.oldImageUrl}, {$set: {imageUrl: req.body.newImageUrl}}, function(err) {
    if (err) {
      return res.status(400).send({errors: [{title: 'Failed to Update Image', detail: err.message}] });
    } else {
      return res.status(200).send({
        message: "Image added",
        success: true
...
router.post("/replace-with-placeholder", (req,res) => {
  ...
  Image.updateOne({imageUrl: req.body.oldImageUrl}, {$set: {imageUrl: "https://aip-project2019.s3.ap-southeast-2.amazonaws.com/1570419663723"}}, function(err) {
    if (err) {
      return res.status(400).send({errors: [{title: 'Failed to Replace Image', detail: err.message}] });
    } else {
      return res.status(200).send({
        message: "Image added",
        success: true
  ...
  Image.findOne({imageUrl: req.body.parentImageUrl}, function(err, document) {
    if(document) {
      ...
      document.save(function(err) {
        if (err) {
          return res.status(400).send({errors: [{title: 'Failed add response image', detail: err.message}] });
        } else {
          return res.status(200).send({
            message: "Image added",
            success: true
